...
В рамках этой статьи мы будем рассматривать два вида таких макросов: derive-макросы, позволяющие автоматически реализовать трейт для какой-то структуры (многим они уже знакомы по [`serde`]), и атрибутные макросы, которые можно для большего спектра задач.
...
Чтобы иметь возможность автоматически реализовывать этот трейт, нам потребуется процедерный макрос.
